...
int CurveFlowForce::getDegreesOfFreedom()
{
    return dimension*(n+2);
}

void CurveFlowForce::setInitialCondition(double* u, void (*func)(double*, double))
{
    /**
...
void CurveFlowForce::getPartialLength(const double* u)
{
    for(int i = 1; i < n+1; i++)
    {
        d[i] = 0;
...
bool CurveFlowForce::writeSolution(const double &t, int step, const double *u)
{
    std::fstream file;
    if(step == 0)
